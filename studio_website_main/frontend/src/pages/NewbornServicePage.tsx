    ...
    async function fetchData() {
      try {
        ...
        if (priceRes.ok && priceData.data) {
          const item = priceData.data.find((p: any) => p.id === 'newborn');
          if (item) {
            setPrices({
      ...
      try {
        ...
        if (galleryRes.ok && galleryData.data) {
          const serviceImages = galleryData.data.filter(
            (img: any) => img.serviceType === 'newborn' && img.servicePosition >= 1 && img.servicePosition <= 4
          );
          ...
          <div className={styles.portfolioGrid}>
            {portfolioImages.map((src, idx) => (
              <div key={idx} className={styles.portfolioCard}>
                <Image
                  src={src}
                  alt={`Newborn portfolio showcase ${idx + 1}`}
                  fill
